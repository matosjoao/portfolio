<x-app-layout>
    ...
    -->
    <x-card>
        <h1 class="font-bold text-lg pb-10 w-4/5">{{ __('Editar Prémio/Certificado') }}</h1>
        ...
        <form method="POST" action="/admin/certificate/{{ $certificate->id }}">
            ...
            <div>
                ...
                <x-input id="title" class="block mt-1 w-full" type="text" name="title" :value="old('title', $certificate->title)" required autofocus />
            </div>
            <!-- Subtitle -->
            <div class="mt-5">
                <x-label for="subtitle" class="font-bold" :value="__('Subtítulo')" />

                <x-input id="subtitle" class="block mt-1 w-full" type="text" name="subtitle" :value="old('subtitle', $certificate->subtitle)" required />
            </div>
            <!-- Certificated Date -->
            <div class="flex mt-5">
                <div class="w-1/2">
                    ...
                    <x-input id="certificated_at" class="block mt-1 w-full" type="date" name="certificated_at" :value="old('certificated_at', $certificate->certificated_at)" required />
                </div>
                <div class="w-1/2 ml-5">
                    <x-label for="icon_name" class="font-bold" :value="__('Icon')" />

                    <x-input id="icon_name" class="block mt-1 w-full" type="text" name="icon_name" :value="old('icon_name', $certificate->icon_name)" required />
                </div>
            </div>
